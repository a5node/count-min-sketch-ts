...
import { createCountMinSketch } from '../src';
...
describe('Testing customisable implementation', () => {
  describe('Basic operations', () => {
    let sketch;
    beforeAll(() => {
      sketch = createCountMinSketch(10, 6, [APHash, JSHash, SDBMHash, DJBHash, DEKHash, APHash]);
    });
  ...
  describe('Testing customisable implementation to/from JSON', () => {
    let sketch;
    let from;
    const obj = { test: 'test', key: 123 };
    beforeAll(() => {
      ...
      from = createCountMinSketch(10, 7, [ELFHash, JSHash, SDBMHash, DJBHash, DEKHash, APHash]);
      from.fromJSON(x);
    });
    describe('With number', () => {
